...
class InvokePlugin implements PluginInterface
{
    ...
    private function verifyErrCode(int $errcode): string
    {
        return match ($errcode) {
            ...
            40005 => "不合法的type参数，参考：上传临时素材",
            40006 => "不合法的文件大小，参考：上传的媒体文件限制",
            40007 => "不合法的media_id参数",
            40008 => "不合法的msgtype参数，参考：消息类型",
            40009 => "上传图片大小不是有效值，参考：上传的媒体文件限制",
            40011 => "上传视频大小不是有效值，参考：上传的媒体文件限制",
            40013 => "不合法的CorpID，需确认CorpID是否填写正确，在 web管理端-设置 可查看",
            40014 => "不合法的access_token",
            40016 => "不合法的按钮个数-3个",
            40017 => "不合法的按钮类型，参考：按钮类型",
            ...
            40024 => "不合法的子菜单按钮类型，参考：按钮类型",
            40025 => "不合法的子菜单按钮名字长度，参考：按钮类型",
            40026 => "不合法的子菜单按钮KEY长度",
            40027 => "不合法的子菜单按钮URL长度，长度应不超过1024字节",
            40029 => "不合法的oauth_code",
            40031 => "不合法的UserID列表，指定的UserID列表是否拼写正确，是否都在应用可见范围之内",
            ...
            40068 => "不合法的标签/标签组ID，标签/标签组ID未指定，或者指定的标签/标签组ID不存在",
            40070 => "指定的标签范围节点全部无效",
            ...
            40203 => "请求参数已废弃，微盘接口请检查userid已废弃",
            40204 => "微信反垃圾",
            40205 => "成员微信票据过期，请确保成员最近一年内或离职前一年内登录过企业微信并进行微信授权",
            40206 => "请求body字节数超过限制",
            40207 => "不合法的tfa_code",
            ...
            42012 => "jsapi_ticket不可用，一般是没有正确调用接口来创建jsapi_ticket，如果是agentConfig使用，请特别注意是否是使用'获取应用身份的ticket'来获取jsapi_ticket。获取到的jsapi_ticket不允许跨应用使用。",
            ...
            42052 => "msgid已过期，仅可撤回24小时内的消息",
            43004 => "指定的userid未绑定微信或未关注微信插件（原企业号），需要成员使用微信登录企业微信或者关注微信插件才能获取openid",
            43009 => "企业未验证主体",
            43012 => "应用需配置回调url",
            43016 => "未获取到用户登录信息，用户须登录后才可访问opendata",
            ...
            48008 => "API接口无权限调用，通讯录编辑授权只能调用部分通讯录接口，查看可调用接口列表",
            48009 => "API接口无权限调用，为保障企业数据安全，不再允许通讯录同步助手从新增IP读取通讯录详情，查看通讯录同步接口调整",
            48010 => "使用了不支持的TLS版本",
            49004 => "签名不匹配",
            ...
            60030 => "已超出应用可见范围，调整应用可见范围覆盖操作的规则组、会议室等",
            60031 => "当前应用已禁止调用API",
            60102 => "UserID已存在",
            ...
            60140 => "无效的response_code，可能是response_code已经过期",
            ...
            60267 => "时间参数不合法，开始时间不能早于180天前；结束时间需要大于开始时间，并且时间跨度不能超过7天",
            60268 => "cursor参数不合法，检查是否使用上次调用本接口时返回的cursor，并且没有改动过滤条件",
            60269 => "企业邮箱别名非法，检查是否为专属域名的邮箱",
            60270 => "企业邮箱别名已存在，企业邮箱别名需全局唯一",
            60271 => "企业邮箱别名非法，检查是否为专属域名的邮箱",
            60272 => "成员没有专属域名邮箱，不能设置企业邮箱别名，需要先为成员分配专属邮箱域名邮箱",
            60273 => "别名邮箱超过限制，不能设置企业邮箱，需要删除部分别名邮箱",
            65000 => "学校已经迁移",
            65001 => "无效的关注模式",
            ...
            84005 => "第三方应用不存在，检查access_token和应用id是否正确",
            84014 => "成员票据过期",
            84015 => "成员票据无效，确认user_ticket参数来源是否正确。参考接口：获取访问用户身份",
            ...
            86006 => "群成员数过多或过少",
            86007 => "不合法的群成员",
            86008 => "非法操作非自己创建的群，请检查是否跨应用，chatid所代表的群必须是该应用所创建",
            86101 => "仅群主才有操作权限",
            86201 => "参数 需要chatid",
            86202 => "参数 需要群名",
            86203 => "参数 需要群主",
            ...
            86225 => "不允许删除课程群群主，课程群删除群主的时候必须指定新群主",
            ...
            90416 => "超级管理员没有权限",
            90418 => "指定的提现人员未实名认证",
            90419 => "上传图片出现错误，请重新上传图片并使用新获得的mediaid，仍失败时请尝试使用新的申请单号重新提交",
            ...
            90602 => "会议Id不合法，检查会议id是否拼写正确，并且为本应用创建的会议",
            90603 => "事件分类id不合法",
            90604 => "网格单元id不合法",
            ...
            95003 => "发送客服消息可接待客户咨询数限制，企业未验证",
            95004 => "open_kfid不存在，跨企业使用，或对已删除的账号操作",
            95005 => "客服账号数超过限制，客服账号数不能多于最大的数量限制，且至少有一个",
            95006 => "不合法的客服账号名",
            95007 => "不合法的msgtoken，msgtoken有时效性，可能为失效或者非法",
            95008 => "菜单消息的菜单项个数超过上限，目前最多允许10个菜单项",
            95009 => "不合法的菜单消息的菜单项类型",
            95011 => "已在企业微信使用微信客服，微信客服当前处于联合版模式，而调用接口的access_token是通过独立版secret获取的",
            95012 => "未在企业微信使用微信客服，微信客服当前处于独立版模式，而调用接口的access_token是通过联合版secret获取的",
            95013 => "会话已经结束",
            ...
            95019 => "接待人员已停止接待或暂时挂起，无法完成指定的操作，可通过获取接待人员列表获取接待人员的状态",
            95020 => "接待人员配置超过上限",
            95022 => "location_type非法",
            95023 => "基础应用权限下，api开关已经授权给代开发自建应用，可通过管理端-微信客服进行权限切换",
            95026 => "客服账号使用率较低，暂无法创建，请改善后再试",
            95027 => "企业未认证，仅可创建10个客服账号，认证企业后即可继续创建",
            95030 => "客服组件设置的禁发消息类型",
            95031 => "客户48小时内未发起过咨询，需要客户发起咨询后才可调用",
            95032 => "客服组件获取客服聊天记录权限已失效，请联系官方对接人员沟通处理",
            95033 => "msgid重复",
            301002 => "无权限操作指定的应用",
            ...
            301019 => "文件的md5不合法，检查文件的md5是否计算正确",
            ...
            301063 => "参数错误",
            301064 => "内部错误",
            301073 => "设置排班的时间参数不合法，openapi目前仅支持对未来日期设置排班",
            301079 => "审批单假勤时间有冲突",
            ...
            301085 => "docid非法",
            301086 => "参数错误，错误内容参考返回的errmsg信息",
            301087 => "审批模板数超过上限",
            301088 => "无操作权限",
            301089 => "groupid非法",
            301090 => "打卡规则包含无权限id，打卡规则中包含不在应用可见范围内的userid/tagid/partyid",
            301091 => "打卡规则range冲突，打卡规则range参数与已有打卡规则冲突",
            301092 => "应用无法修改此打卡规则，打卡规则仅支持由规则的创建应用修改",
            301093 => "打卡规则字段非法，打卡规则字段非法，报错指引",
            301094 => "打卡规则字段非法，打卡规则字段非法，包含有非法字段，报错指引",
            301095 => "企业打卡规则总数超限，企业已有打卡规则总数超出限制，无法添加",
            301096 => "创建打卡规则并发超限，创建打卡规则并发超限，请重试",
            301097 => "打卡规则字段非法，打卡规则中位置和wifi信息不能同时为空",
            ...
            301112 => "请缩小查询时间范围重试",
            301113 => "审批中的审批打卡不能补卡，补卡后原审批打卡的信息会清除，排查预期补卡的记录是否为审批中的审批打卡",
            ...
            400041 => "会议已取消",
            400042 => "会议title不合法，长度不符合要求",
            400043 => "已废弃该参数，请使用admin_userid",
            400044 => "系统应用仅支持creator_userid参数，系统应用需要将admin_userid参数替换为creator_userid",
            ...
            400301 => "5分钟内有相同的会议正在创建中，暂不可再创建，若相同的会议未创建成功，可于相同会议首次创建时间5分钟后重新创建",
            400302 => "相同的会议已经创建成功，若明确需要创建相同的会议，可在创建会议请求包中设置'skip_duplicate_check':true 跳过重复会议检查",
            400303 => "用户没有权限创建会议，可以联系企业管理员在腾讯会议管理端为用户配置创建会议的权限",
            400304 => "用户没有操作权限，确认用户是否已经激活且使用过日历",
            400307 => "不允许获取其他企业成员创建的会议详情，检查会议是否为本企业成员创建",
            ...
            610023 => "企业外部联系人规模已达上限，当前已无法管理客户，在客户端上购买后即可继续管理。，去管理后台购买",
            ...
            640035 => "免费账号调用微盘api累计上限为1000次/月，每个企业内所有免费账号共用此次数限制，升级为付费账号，不受此限制",
            640039 => "分块上传的size和block_sha不匹配，按此指引计算并填入正确的block_sha以及对应的size",
            640044 => "空间已开启保密模式，检查空间相关设置",
            640045 => "没有权限设置“可上传下载”权限，检查操作者账号类型",
            640046 => "没有权限设置“自定义”权限，检查操作者账号类型",
            640047 => "上传文件大小超过限制，单个文件大小不能超过20G",
            640048 => "上传流程异常，不能重复调用file_upload_finish",
            640051 => "参数错误，检查参数是否符合API文档的要求",
            640052 => "文档名不合法，请检查文档名是否符合API文档的要求",
            640053 => "管理员数量达到上限，请减少管理员的数量，最多为3个。",
            640054 => "文档类型不合法，请检查文档类型是否符合API文档的要求",
            640055 => "文档成员达到上限，请减少文档成员的数量",
            640056 => "欲删除的成员当前不在文档成员列表中，请核对欲删除的成员和现有成员",
            640060 => "空间文件数已达上限，清理空间下不再使用的文件",
            640061 => "智能表api内容权限规则名称重复，这是说明传入的name参数有重复或者name=“全员权限”，这些都是不被允许的，请修改name参数",
            640062 => "智能表api内容权限规则名称长度超限，这是说明传入的name参数长度超过限制，目前限制的name长度为20个字符",
            640063 => "智能表api指定成员权限规则数量超限返回，这是成员权限规则超过限制，每个文档的额外成员权限规则数量限制为20个",
            640065 => "智能表api额外指定成员权限不存在，传入的规则id不存在，请检查输入的ruleid是否合法",
            640067 => "智能表api删除的成员不存在，删除的成员非法，该成员不在当前的权限成员列表",
            640069 => "智能表api规则成员数目超限，智能表额外成员权限规则限制添加的成员人数不得大于500人",
            640070 => "智能表api设置区域权限-全局行规则不能设置局部行规则，rule_list.priv_list.record_priv.record_range_type=1标识全部记录，则不能再设置rule_list.priv_list.record_priv.record_rule_list和rule_list.priv_list.record_priv.record_rule_list.other_priv",
            640071 => "智能表api设置区域权限-行规则数目非法，rule_list.priv_list.record_priv.record_rule_list的长度为0或者超过限制20",
            640072 => "智能表api设置区域权限-行规则缺少fieldid，rule_list.priv_list.record_priv.record_rule_list的某一项缺少字段id，请检查rule_list.priv_list.record_priv.record_rule_list.field_id",
            640073 => "智能表api设置区域权限-行规则操作类型无需有值，rule_list.priv_list.record_priv.record_rule_list.oper_type的取值等于1-包含自己（人员字段）、6-为空、7-非空时候，rule_list.priv_list.record_priv.record_rule_list.value不能有值",
            640074 => "智能表api设置区域权限-行规则操作类型必须有值，rule_list.priv_list.record_priv.record_rule_list.oper_type的取值等于2-包含value；3-不包含value；4-等于value；5-不等于时候valuerule_list.priv_list.record_priv.record_rule_list.value必须有值",
            640075 => "智能表api设置区域权限-行规则操作类型未定义，情况1：rule_list.priv_list.record_priv.record_rule_list.oper_type的取值不在[1-7]之间，情况2：rule_list.priv_list.record_priv.record_rule_list.field_id = CREATED_USER 但是 rule_list.priv_list.record_priv.record_rule_list.oper_type不等于1-包含自己（人员字段），情况3：rule_list.priv_list.record_priv.record_rule_list.field_type为人员列FIELD_TYPE_USER 但是 rule_list.priv_list.record_priv.record_rule_list.oper_type不等于1-包含自己（人员字段）",
            640076 => "智能表api设置区域权限-行规则其他权限非法，情况1：priv_list.record_priv.record_rule_list.other_priv的取值不在[1-2]之间，情况2：priv_list.priv设置为3-仅浏览，但是priv_list.record_priv.record_rule_list.other_priv的取值不为2-不可查看",
            640077 => "智能表api设置区域权限-行规则类型未定义，priv_list.record_priv.record_range_type的取值不在[1-3]之间",
            640078 => "智能表api设置区域权限-全局列规则不能设置局部列规则，priv_list.field_priv.field_range_type = 1-所有字段，则不能再设置priv_list.field_priv.field_rule_list 和priv_list.field_priv.field_default_rule",
            640079 => "智能表api设置区域权限-列规则缺少fieldid，设置字段规则时，field id为空",
            640080 => "智能表api设置区域权限-列规则fieldid重复，priv_list.field_priv.field_rule_list.field_id重复设置",
            640081 => "智能表api设置区域权限-列规则非法，情况1：priv_list.priv的设置级别和priv_list.field_priv.field_rule_list.can_edit，priv_list.field_priv.field_rule_list.can_insert，priv_list.field_priv.field_rule_list.can_view，之间互相冲突，请校验这些地方是否符合正常逻辑，不要出现矛盾的设置，一般外层权限大于内层权限，情况2：最小权限和列规则的三个can之间冲突，最小权限指的是当priv_list.priv设置为2-可编辑；3-仅浏览时候，最小权限会根据priv_list.can_insert_record的设置为，最小权限为可插入记录，还是最小权限为仅浏览，请检查是否与列规则的三个can之间冲突",
            640082 => "智能表api设置区域权限-列规则最小权限非法，这种情况是因为填充了行规则需要某个字段列，但是列规则里面没有设置这个列的权限，会导致判断字段权限时候缺失",
            640083 => "智能表api设置区域权限-列规则默认权限非法，全员权限不能设置priv_list.field_priv.field_default_rule，额外成员权限必须设置priv_list.field_priv.field_default_rule",
            640084 => "智能表api设置区域权限-列规则can相关操作权限非法，priv_list.field_priv.field_default_rule的can操作之间相冲突或者和外层的priv_list.priv设置级别冲突",
            640085 => "智能表api设置区域权限-列规则类型未定义，priv_list.record_priv.record_range_type的取值不在[1-3]之间",
            640086 => "智能表api设置区域权限-子表id为空，检查priv_list.sheet_id是否为空",
            640087 => "智能表api设置区域权限-子表id重复，检查priv_list.sheet_id是否重复",
            640088 => "智能表api设置区域权限-子表priv非法，情况1：priv_list.priv设置为1-全部权限和4-无权限时，不能设置同级别的其他字段，包括field_priv和record_priv，情况2：priv_list.priv设置为2-可编辑，必须有field_priv和record_priv以及can操作，情况3：priv_list.priv设置为3-仅浏览，必须有field_priv和record_priv，但是不能有can操作",
            640089 => "智能表api设置区域权限-子表priv.priv类型非法，priv_list.priv的取值不在[1-4]之间",
            640090 => "智能表api设置区域权限-成员类型非法，修改额外权限的成员列表时，输入的成员id类型不合法，请检查输入的成员类型是否匹配对应的入参列表",
            640091 => "智能表api设置区域权限-成员重复，修改额外权限的成员列表时，输入的成员id重复",
            640092 => "智能表api设置区域权限-成员非法，删除成员列表的成员id在当前权限的成员列表中不存在",
            640093 => "智能表api设置列类型list为空，设置列权限为部分字段可编辑时，没有传入具体字段priv_list.field_priv.field_range_type=2-部分字段时，没设置priv_list.field_priv.field_rule_list",
            640094 => "智能表api全员权限不能设置名称，全员权限不能修改名称",
            640095 => "智能表api创建者列类型非法，设置记录权限时，若priv_list.record_priv.record_rule_list.field_id=CREATED_USER，则不能设置priv_list.record_priv.record_rule_list.field_type字段",
            640096 => "智能表api列类型非法，设置记录权限时，缺失priv_list.record_priv.record_rule_list.field_type字段或者是列类型不在['FIELD_TYPE_USER, 'FIELD_TYPE_SELECT', 'FIELD_TYPE_SINGLE_SELECT', 'FIELD_TYPE_LOOKUP']之间，设置字段权限时，缺失priv_list.field_priv.field_rule_list.field_type字段",
            640097 => "智能表api表格id非法，输入的rule_list.priv_list.sheet_id子表id不存在，请校验子表id",
            640098 => "智能表api表格日期数字文件列设置错误，日期、数字和文件列这3个规则联动，不存在只勾可编辑+可查看",
            640100 => "智能表api表格引用列设置错误，引用、公式不可编辑，可以不可查看",
            640101 => "智能表api表格系统属性列设置错误，系统字段不可编辑，最小可查看系统字段指的是创建人、修改人、创建时间和修改时间这四个字段",
            ...
            670005 => "超过上下游人企业数上限",
            670006 => "无效上下游规则id",
            670007 => "上下游规则id数量超出限制",
            670008 => "分组不唯一",
            670009 => "自定义id不唯一",
            670010 => "负责人数量超出限制",
            670011 => "不合法的自定义id。上下游企业自定义id只支持64个字符以内的字母和数字",
            670012 => "导入企业名称为空",
            670013 => "不合法的企业名称。企业名只支持30个字符以内的中/英文、数字、合法字符",
            670014 => "上下游联系人姓名为空",
            670015 => "不合法的上下游联系人姓名",
            670016 => "不合法的身份字段。身份字段只支持填写负责人或成员",
            670017 => "分组名和同层级企业名重复",
            670018 => "该联系人账号异常，无法邀请其加入上下游",
            ...
            670028 => "企业名称不唯一",
            670029 => "成员、部门不在上下游范围",
            670030 => "无权限操作此groupid。检查groupid是否在应用共享范围内",
            670031 => "根据管理员设置，存在联系人无权加入其他企业创建的上下游",
            670032 => "成员自定义id不合法",
            670033 => "当前成员填写了多个自定义id",
            670034 => "成员自定义id跟当前下游企业其他成员重复",
            670035 => "成员没有设置自定义id",
            680000 => "参数错误，结合返回的errmsg排查",
            ...
            680006 => "群组使用权限，仅支持0: 企业成员, 1任何人， 2:组内成员，3:自定义成员",
            680007 => "允许使用群组成员邮箱非法，检查群组成员邮箱格式是否正确",
            680008 => "群组成员为空，检查userlist/grouplist/department是否都为空",
            680010 => "邮件群组地址重复，检查群组地址在企业内是否已经被用户占用",
            680011 => "邮件群组地址重复，检查群组地址在企业内是否已经被其他群组占用",
            680012 => "邮件群组不存在，检查群组地址是否有误，是否存在于本企业",
            680015 => "模糊搜索fuzzy参数非法，检查fuzzy值是否为0或1",
            680032 => "业务邮箱地址非法，检查邮箱地址格式是否正确，是否为本企业域名",
            680033 => "业务邮箱地址重复，检查业务邮箱地址在企业内是否已经被占用",
            680034 => "业务邮箱名称非法，名称是否为空，名称是否过长",
            680035 => "业务邮箱ID不存在，检查id是否存在",
            680036 => "业务邮箱ID非法，检查id数据类型是否正确，是否传值",
            680037 => "vid账号余量不足，检查vid账号数是否用完",
            680040 => "业务邮箱名称重复，检查名称是否已经被其他业务邮箱占用",
            680041 => "获取用户功能属性type字段非法，仅支持1: 强制启用安全登录 2: IMAP/SMTP服务 3: POP/SMTP服务 4: 是否启用安全登录",
            680042 => "更改用户功能属性type字段非法，仅支持1: 强制启用安全登录 2: IMAP/SMTP服务 3: POP/SMTP服务 4: 是否启用安全登录",
            680043 => "禁用/启用邮箱时type参数非法，仅支持 1:启用，2:禁用",
            680044 => "不允许禁用超级管理员、企业创建者，检查userid是否为超级管理员或企业创建者",
            680045 => "无法禁用已关闭的邮箱，检查邮箱账号是否为关闭",
            680046 => "启用邮箱数量达到最大限制，检查企业中已启用的邮箱数量",
            680047 => "别名邮箱格式不合法",
            680048 => "别名邮箱账号已存在",
            680049 => "传入的别名邮箱超出5个数量上限",
            680050 => "企业没有专属域名，不允许添加别名",
            680106 => "当前企业会议应用已关闭，音视频会议不可用，请联系管理员",
            680151 => "客户端专用密码id不正确",
            680153 => "客户端专用密码个数超过上限",
            ...
            701007 => "不合法的账号类型",
            701008 => "没有合法的有互通license的用户，该userid没有激活互通账号，请激活后重试",
            ...
            701034 => "历史企业，需迁移完成后才能购买license订单，可以去服务商管理端购买license订单页面点击主动迁移",
            701035 => "有正式订单，所以无法添加测试订单",
            701036 => "有生效中的测试订单，所以无法添加订单",
            701037 => "存在订单申请退款中",
            701038 => "民生企业，只允许下单12个月",
            701039 => "民生企业，不允许续期",
            ...
            701082 => "该用户已经激活的情况下，使用新码重新激活的场景，旧码剩余的时长不能超过20天",
            701083 => "订单未支付",
            701084 => "订单不是未支付",
            701085 => "订单无法操作，请稍后再试",
            701086 => "订单不属于该企业",
            ...
            701089 => "用户不在服务商互通权限（「客户联系」或「微信客服」权限）应用的可见范围内",
            701090 => "续期订单中全部账号的续期天数都等于0",
            701099 => "用户没有有效的接口许可，接口许可失效的表现，参考该文档排查是否全部待检查用户都有有效的接口许可",
            701100 => "时间戳不合法，开始时间必须小于结束时间；时间范围不能超过14天；非微盘专业版的企业只能查询90天内的数据",
            701101 => "limit大小超过上限1000",
            701102 => "查询用户数超过上限100",
            701103 => "操作类型不合法",
            701104 => "操作来源不合法",
            701105 => "cursor不合法，检查是否企业微信返回的cursor",
            701107 => "用户不在文档内容存档范围或者不在应用可见范围",
            701108 => "docid不合法",
            701109 => "导出格式不合法",
            701112 => "民生行业优惠政策已到期",
            701120 => "该接口不支持传父订单id",
            ...
            701131 => "无应用订单，处于免费试用状态，但购买账号码数超过免费试用的人数上限",
            701132 => "购买时长不大于3个月，购买账号数不允许超过通讯录人数的2倍",
            701133 => "无应用订单，处于免费试用状态，购买时长不允许超过3个月（93天）",
            701134 => "操作过于频繁，请稍后再试",
            701135 => "接口许可购买时长不可低于应用订单时长",
            701139 => "未授权会话存档应用，无法创建会话存档订单",
            701140 => "已经购买过此版本会话存档账号，不能再新购，只能续期、扩容、升级",
            701141 => "未购买此版本会话存档账号或已经过期，不能增购、续期、升级",
            701142 => "订单可能被其他并发请求更新，请重试",
            701143 => "在未支付或申请退款中的订单，不能下单",
            701146 => "海外企业不能购买会话存档",
            701147 => "余额支付超过单次支付限额上限",
            701149 => "只有临过期60天可以续期",
            701150 => "测试企业不支持分配许可",
            701151 => "jobid不正确或者已失效",
            701152 => "docid不正确或者对应的文档不是公开状态",
            ...
            701163 => "合法的生效时间",
            701164 => "不存在可升级的版本",
            701165 => "少chat_archive_api字段",
            701166 => "操作者没有权限，或者已离职、退出、禁用",
            701167 => "购买人数超出上限",
            701169 => "只允许取消未支付的订单",
            701170 => "群活码第三方限免许可到期",
            701200 => "其他镜像发布中，禁止发布",
            701230 => "指定的原始的开始或者结束时间有误",
            701231 => "存在待生效的版本",
            701250 => "时间戳不合法，开始时间必须小于结束时间；时间范围不能超过14天。",
            701252 => "cursor不合法",
            ...
            710401 => "硬件云端接入设备model_accesstoken过期，model_access_token有时效性，需要重新获取一次",
            ...
            710602 => "msgtype不正确",
            710603 => "密钥信息不正确",
            710605 => "消息解密失败，检查msgid和密钥信息是否正确",
            710606 => "群ID不在授权范围内",
            710607 => "部门不在授权范围内",
            710608 => "用户不在授权范围内",
            ...
            710616 => "提交分析的任务类型不正确，检查本次提交的任务类型与jobid对应的任务类型是否一致",
            710617 => "分析任务已经提交过了",
            710618 => "分析任务还未提交",
            710619 => "关键词规则数量已经超过上限，上限详见：关键词规则管理",
            710620 => "关键词规则ID不正确",
            710621 => "电话号码格式不正确",
            710622 => "银行卡号格式不正确",
            710623 => "单次分析任务的消息数量超过上限，上限详见：会话内容分析",
            710624 => "不合法的群ID",
            710625 => "分析任务类型不正确",
            710626 => "自定义ID已存在",
            710627 => "空间不足",
            ...
            710660 => "数据专区示例镜像内部错误，根据errmsg以及专区日志进一步排查",
            710671 => "调试凭证已过期",
            ...
            730006 => "非法的报名id",
            730007 => "企业不是专业版或购买的会议室数量不足10个，参考会议接口文档概述",
            730008 => "会议已删除",
            730009 => "会议参与者必须包含管理员",
            730010 => "会议不是通过API创建的",
            730011 => "不支持该类型的会议，不是通过API创建的",
            740000 => "安全网关gateway_info的<host,port>冲突，企业内<host,port>唯一",
            740001 => "安全网关target_info的path冲突，企业内path唯一",
            740002 => "安全网关数量超过上限，企业内最多10个",
            740003 => "单个安全网关内的path数量超过限制，单个网关内最多100个",
            740004 => "安全网关不存在，可能已被删除",
            740005 => "安全网关gateway_info的host非域名，目前要求必须是域名，不能配置ip",
            740006 => "安全网关gateway_info的host验证失败，要求域名ICP备案跟企业主体一致",
            751001 => "企业未开通邮箱高级功能",
            ...
            790032 => "非法的知识库內容名称，检查doc_name的长度是否过长，后缀是否和文件格式匹配等",
            790033 => "名字中不能包含不可见字符",
            790034 => "不能使用已删除的知识库",
            ...
            830001 => "用于上传临时素材的url非法，确认url是否支持Range分块下载",
            830002 => "下载的文件太大，或指定的Range范围太大，确认下载的文件大小，如果超过20M，需要使用Range分块下载，且分块大小不超过20M",
            830003 => "用过上传临时素材的url下载数据失败，确认url本身是否能正常访问",
            842002 => "代开发应用模版未上线",
            842003 => "不是代开发应用模版",
            842004 => "代开发应用模版数量不合法",
            ...
            844002 => "最近安装应用时间已超过7天，不再允许拨打公费电话联系管理员，请检查最近一次安装应用时间是否超过7天",
            845001 => "openid账号类型不是公众号或小程序",
            845002 => "openid认证主体和企业认证主体不一致，请确保小程序或公众号已经认证，且认证的主体名称与企业的主体名称一致",
            845003 => "unionid认证主体和企业认证主体不一致，请确保开放平台账号已经认证，且认证的主体名称与企业的主体名称一致",
            846000 => "不是代开发或者第三方应用",
            ...
            2022029 => "智能表的 行|列|大小 达到上限",
            2022030 => "不合法的字段标题，重复、为空等，检查标题是否为空、重复",
            2022031 => "ID已经存在，检查视图、字段、记录ID是否已经存在",
            2022032 => "不合法的视图属性，合法的视图属性，参考：viewproperty",
            2022033 => "不合法的筛选条件，合法的筛选条件，参考：filterspec",
            2022034 => "不合法的日期值，合法的日期值，参考：添加记录",
            2022035 => "不合法的关联值，合法的关联值：参考：",
            2022037 => "不合法的地理位置值，合法的地理位置值，参考：celllocationvalue",
            2022040 => "不合法的字段列属性，合法的字段列属性，参考：添加字段",
            2022041 => "不合法的关联字段列属性，合法的关联字段列属性，参考：referencefieldproperty",
            2022043 => "不合法的地址位置字段列属性，合法的地理位置字段列属性，参考：locationfieldproperty",
            2022047 => "不合法的自动编号字段列属性，合法的自动编号字段列属性，参考：autonumberfieldproperty",
            2022048 => "没有编辑权限，确认是否有文档权限和内容权限",
            2022049 => "当前视图已被锁定，无法修改视图结构。，确认当前视图的公开级别",
            2022050 => "当前视图不可见，无法修改视图结构。，确认当前视图的公开级别",
            2022051 => "无法修改应用关联的记录",
            2022052 => "最多固定 3 个字段到工具栏",
            2400001 => "请求参数错误",
            2400003 => "访问的资源不存在",
            2400004 => "创建的资源已存在",
            2400005 => "不允许访问该资源",
